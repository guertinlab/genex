...
```{r  out.width = "100%", out.height = "100%", echo=F, fig.align = "center", fig.cap="ATAC-seq molecular biology"}
library(knitr)
...
We already named the experiments with the trailing name being `PE1` or `PE2` for paired end data. You can retrieve the files by substituting you group number (1-8) for `XX` in the code below. Actually I don't think these links work / we'll skip the steps you already performed in the ChIP-seq analysis and start with the differential accessibility analysis in R below.
...
mkdir -p ~/genex_ATAC/
cd ~/genex_ATAC/
wget http://guertinlab.cam.uchc.edu/HEK293T_ATAC_DMSO_repXX_PE1.fastq.gz
wget http://guertinlab.cam.uchc.edu/HEK293T_ATAC_DMSO_repXX_PE2.fastq.gz
wget http://guertinlab.cam.uchc.edu/HEK293T_ATAC_DTAG_repXX_PE1.fastq.gz
wget http://guertinlab.cam.uchc.edu/HEK293T_ATAC_DTAG_repXX_PE2.fastq.gz
```
...
do
	...
	fastq_pair -t $reads ${name}_PE1.chrM.fastq ${name}_PE2_no_adapt.fastq 2>&1 | tee ${name}_fastq_pair.log
done
```
...
We next call peaks with `macs2` using all the `*.hg38.bam` files. We
...
peak_qvalue = x[,5]
x = x[,-c(1:6)]
colnames(x) = sapply(strsplit(sapply(strsplit(colnames(x), "HEK293T_"), "[", 2), ".hg38"), "[", 1)

sample.conditions = factor(sapply(strsplit(colnames(x), '_'), '[', 1), levels=c("DMSO","DTAG"))
...
plotPCAlattice(pca.plot, file = 'PCA_plot_30min_ZNF143_degradation.pdf')
...
ma.plot.lattice(DE.results.lattice, filename = '30min_ZNF143_degradation', 
...
write.table(cbind(chr, start, end), file = "ATAC_decreased_ZNF143_degron.bed", quote = FALSE,
col.names =FALSE, row.names=FALSE, sep = "\t")

unchanged.all = DE.results.lattice[DE.results.lattice$response == 
                                             'ZNF143_degron_30min Unchanged',]
...
write.table(cbind(chr.un, start.un, end.un), file = "ATAC_unchanged_ZNF143_degron.bed", quote = FALSE,
col.names =FALSE, row.names=FALSE, sep = "\t")
```

```{r  out.width = "50%", out.height = "50%", echo=F, fig.align = "center", fig.cap="PCA Analysis"}
library(knitr)
knitr::include_graphics("PCA_plot_30min_ZNF143_degradation.png") 
```

```{r  out.width = "50%", out.height = "50%", echo=F, fig.align = "center", fig.cap="MA plot"}
library(knitr)
knitr::include_graphics("MA_plot_30min_ZNF143_degradation.png") 
```
...
fastaFromBed -fi $genome -bed ATAC_decreased_ZNF143_degron.bed -fo ATAC_decreased_ZNF143_degron.fasta
fastaFromBed -fi $genome -bed ATAC_unchanged_ZNF143_degron.bed -fo ATAC_unchanged_ZNF143_degron.fasta

meme -oc ZNF143.meme_output -objfun classic -evt 0.01 -searchsize 0 -minw 5 -maxw 15 -revcomp -dna -markov_order 2 -maxsize 100000000 ATAC_decreased_ZNF143_degron.fasta

mast ZNF143.meme_output/meme.txt -mt 0.001 -hit_list ATAC_decreased_ZNF143_degron.fasta > mast_hits_decreased.txt
mast ZNF143.meme_output/meme.txt -mt 0.001 -hit_list ATAC_unchanged_ZNF143_degron.fasta > mast_hits_unchanged.txt

cut -f 1 -d " " mast_hits_decreased.txt | sort | uniq | grep -v "#" > unique_mast_hits_decreased.txt 
wc -l unique_mast_hits_decreased.txt
wc -l ATAC_decreased_ZNF143_degron.bed

cut -f 1 -d " " mast_hits_unchanged.txt | sort | uniq | grep -v "#" | wc -l
wc -l  ATAC_unchanged_ZNF143_degron.bed
```

```{r  out.width = "50%", out.height = "50%", echo=F, fig.align = "center", fig.cap="ZNF143 motif"}
library(knitr)
