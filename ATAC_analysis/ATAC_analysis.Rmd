...
categorize.DEseq.df <- function(DE.results, unchanged.padj = 0.02, changed.padj = 0.1) {
    ...
    activated = DE.results[DE.results$padj < 0.1 & !is.na(DE.results$padj) & DE.results$log2FoldChange > 0,]
    unchanged.act = DE.results[!is.na(DE.results$padj) & DE.results$padj > 0.1 & abs(DE.results$log2FoldChange) < unchanged.padj,]
    ...
    repressed = DE.results.lattice[DE.results.lattice$padj < 0.1 & !is.na(DE.results.lattice$padj) & DE.results.lattice$log2FoldChange < 0,]
                            ...
                            col = c("grey80", "grey20" , "grey20", "#2290cf", "#ce228e"))
{
    ...
    ma.df$category <- factor(DE.results.lattice$category, levels = desired_order)
...
setwd("/Users/guertinlab/Desktop/ZNF143_ATAC/")
...
ma.plot.lattice(DE.results.lattice, filename = "ATAC_ZNF143_30min_accessiblity_MA.pdf")
...
write.table(cbind(chr, start, end), file = "ZNF143_degron_ATAC_matched.bed", quote = FALSE,
col.names =FALSE, row.names=FALSE, sep = "\t")

                                        #I made this chart manually with intersectBed-v outputs (need to automate)

x = read.table('barchart_ATAC_overlap_ZNF143.txt', sep = "\t", header=TRUE)


#x$factor<- factor(x$factor,
#                     levels = rev(sort(unique(x$factor))),
#                     ordered = TRUE)


plot.barchart(x, filename = "barchart_ATAC_overlap_ZNF143.pdf")
