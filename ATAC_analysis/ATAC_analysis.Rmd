...
curl -O https://hgdownload.cse.ucsc.edu/goldenpath/hg38/chromosomes/chrM.fa.gz
gunzip chrM.fa.gz
...
genome_index=/path/to/indiceis/hg38_bt2
...
# mappability tallymer files were pregenerated using this command:

NEED TO DO THIS

# these can only be used for 40 base reads and hg38, but if you 

#Convert BAM to bed for counting (previously created the tallymer with `seqOutBias seqtable`)
seqOutBias hg38.fa ${name}.hg38.bam --no-scale --custom-shift=4,-4 --read-size=62 2>&1 | tee ${name}_seqOutBias.log
```
...
write.table(cbind(chr, start, end), file = "ATAC_decrease_ZNF143_degron.bed", quote = FALSE,
...
fastaFromBed -fi $genome -bed ATAC_increase_ZNF143_degron.bed -fo ATAC_increase_ZNF143_degron.fasta
fastaFromBed -fi $genome -bed ATAC_unchanged_ZNF143_degron.bed -fo ATAC_unchanged_ZNF143_degron.fasta

meme -oc ZNF143.meme_output -objfun classic -evt 0.01 -searchsize 0 -minw 5 -maxw 10 -revcomp -dna -markov_order 2 -maxsize 100000000 ATAC_increase_ZNF143_degron.fasta

mast ZNF143.meme_output/meme.txt -mt 0.001 -hit_list ATAC_increase_ZNF143_degron.fasta > mast_hits_increase.txt
...
wc -l ATAC_increase_ZNF143_degron.bed
